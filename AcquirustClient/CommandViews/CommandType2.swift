...
class CommandType2: NSTableCellView, NibLoadable {
    ...
    @IBAction func submitCommand(_ sender: Any) {
        guard let amount = Int(operationInput2.stringValue) else {
            responseText.stringValue = "Faild to parse amount as Int"
            return
        }
        HttpClient.shared.openCredit(
            cardNumber: operationInput1.stringValue,
            amount: amount
        ) { response in
